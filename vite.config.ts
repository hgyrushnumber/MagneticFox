...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      // 反向代理
      proxy: {
        // 将这里的路径修改为你希望代理的前端请求路径前缀，比如这里改为 "/api"（可根据实际情况自定义）
        "/api": {
          // 主要出问题就是这里
          // target: "http://www.magneticfox.top:8081",
          // target: "https://www.magneticfox.top/",
          target: "http://localhost:8081",
          changeOrigin: true,
          rewrite: path => path.replace(/^\/api/, "") // 去掉 /api 也就是在@RequestMapping("/api")把这个注释掉
        },
        "/calculate": {
