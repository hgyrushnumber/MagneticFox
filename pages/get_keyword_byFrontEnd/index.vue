...
async function getKeyword(row: TableData) {
  ...
  const requestData: GetKeywordRequestData = {
    productname: productname || "", // 假设 row 中包含 productname 字段
    keywordNumber: top_n || "" // 假设 row 中包含 keywordNumber 字段
  }
  ...
  axios.post("/keyword/getBestKeyword", jsonData, {
    ...
    .then((response) => {
      ...
      const dataToSend = {
        productname: row.productname, // 添加 productname
        keywords: row.keywords // 添加 keywords
      }
      axios.post("/api/insertIntoKeywordInfo", dataToSend, {
        headers: {
          "Content-Type": "application/json"
        }
      })
...
async function deleteGoodList() {
  ...
  try {
    // 准备删除的商品 ID 列表
    const productNumbersToDelete = selectedGoods.value.map(item => item.productname)
...
function addEmptyRecord() {
  ...
  tableData.value.push(emptyRecord) // 将空记录推送到 tableData 中
}
function addEmptyRecordNew(productnameNew: string) {
  console.log("addEmptyRecordNew执行了")
  const emptyRecord: TableData = {
  ...
  console.log("emptyRecord:", emptyRecord) // 检查 emptyRecord
  console.log("tableData before push:", tableData.value) // 检查 tableData
  tableData.value.push(emptyRecord) // 将空记录推送到 tableData 中
  console.log("tableData after push:", tableData.value) // 检查 tableData 是否更新
  // 确保视图更新
  nextTick(() => {
...
onMounted(() => {
  ...
  watch(() => route.query, (newQuery) => {
    const action = newQuery.action
    console.log(`action是${action}`)
    const productname = String(newQuery.商品名称)
    console.log(`商品名称是${productname}`)
    if (action === "addRow" && productname) {
      addEmptyRecordNew(productname)
    }
  }, { immediate: true })
})
// 监听分页参数的变化
        ...
        <el-table :data="tableData" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="productname" label="商品名" align="center">
            <template #default="{ row }">
              <el-input v-model="row.productname" type="text" placeholder="商品名" required />
            </template>
          </el-table-column>
          <el-table-column prop="time" label="添加时间" align="center">
            <template #default="{ row }">
              <el-input v-model="row.time" type="text" placeholder="添加时间" required />
            </template>
          </el-table-column>
